...
document.getElementById(/type){
    let type = document.getElementById('pokemonName').value.toLowerCase();

    fetch(`https://pokeapi.co/api/v2/pokemon/${/type}`)
        .then(response => response.json())
        .then(data => {
            // Manipulando o DOM para exibir as informações
            document.getElementById('types').textContent = data.name.toUpperCase();
            document.getElementById('description').src = data.sprites.front_default;
            document.getElementById('pokemonImage').style.display = "block";
